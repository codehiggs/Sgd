...
$fields = [
			 ...
			 'size' => ['type' => 'DECIMAL', 'null' => FALSE],
			 'type' => ['type' => 'VARCHAR','constraint' =>128, 'null' => FALSE],
			 'url' => ['type' => 'VARCHAR','constraint' =>254, 'null' => FALSE],
...
/**
		...
		{
				$key = $this->get_CacheKey("file-{$file}");
				$cache = cache($key);
				if (!$this->is_CacheValid($cache)) {
						$row = $this->where($this->primaryKey, $file)->first();
						if (is_array($row)) {
								$value = $row;
						} else {
								$value = false;
