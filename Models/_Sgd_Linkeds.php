...
/**
* Ej: $model = model('App\Modules\Sgd\Models\Sgd_Linkeds');
* @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
* @method groupStart() : \Higgs\Database\BaseBuilder
*/
class Sgd_Linkeds extends Model
{
protected $table = "sgd_linkeds";
protected $primaryKey = "linked";
...
$fields = [
			 'linked' => ['type' => 'VARCHAR','constraint' =>13, 'null' => FALSE],
			 'file' => ['type' => 'VARCHAR','constraint' =>13, 'null' => FALSE],
			 'subserie' => ['type' => 'VARCHAR','constraint' =>13, 'null' => FALSE],
			 'metatag' => ['type' => 'VARCHAR','constraint' =>13, 'null' => FALSE],
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						...
						->like("linked", "%{$search}%")
						->orLike("file", "%{$search}%")
						->orLike("subserie", "%{$search}%")
						->orLike("metatag", "%{$search}%")
...
/**
		 ...
		 * @return array|false
		 */
		public function get_Linked($linked):false|array
		{
				$key = $this->get_CacheKey("linked-{$linked}");
				$cache = cache($key);
				if (!$this->is_CacheValid($cache)) {
						$row = $this->where($this->primaryKey, $linked)->first();
						if (is_array($row)) {
								$value = $row;
						} else {
								$value = false;
