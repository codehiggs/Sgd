...
$fields = [
			 ...
			 'value' => ['type' => 'VARCHAR','constraint' =>254, 'null' => FALSE],
...
/**
		...
		{
				$key = $this->get_CacheKey("metatag-{$metatag}");
				$cache = cache($key);
				if (!$this->is_CacheValid($cache)) {
						$row = $this->where($this->primaryKey, $metatag)->first();
						if (is_array($row)) {
								$value = $row;
						} else {
								$value = false;
