...
/**
* Ej: $model = model('App\Modules\Sgd\Models\Sgd_Subseries');
* @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
* @method groupStart() : \Higgs\Database\BaseBuilder
*/
class Sgd_Subseries extends Model
{
protected $table = "sgd_subseries";
protected $primaryKey = "subserie";
...
/**
		 ...
		 * @return array|false
		 */
		public function get_Subserie($subserie):false|array
		{
				$key = $this->get_CacheKey("subserie-{$subserie}");
				$cache = cache($key);
				if (!$this->is_CacheValid($cache)) {
						$row = $this->where($this->primaryKey, $subserie)->first();
						if (is_array($row)) {
								$value = $row;
						} else {
								$value = false;
