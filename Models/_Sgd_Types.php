...
/**
		 ...
		 * @return array|false
		 */
		public function get_Type($type):false|array
		{
				$key = $this->get_CacheKey("type-{$type}");
				$cache = cache($key);
				if (!$this->is_CacheValid($cache)) {
						$row = $this->where($this->primaryKey, $type)->first();
						if (is_array($row)) {
								$value = $row;
						} else {
								$value = false;
