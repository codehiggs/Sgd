...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Types\Delete\form.php]
...
$r= $model->get_Types($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Types.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Types.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Types.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
