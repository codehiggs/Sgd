...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Files\Delete\form.php]
...
$r= $model->get_Files($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Files.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Files.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Files.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
