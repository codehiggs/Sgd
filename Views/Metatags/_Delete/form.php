...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Metatags\Delete\form.php]
...
$r= $model->get_Metatags($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Metatags.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Metatags.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Metatags.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
