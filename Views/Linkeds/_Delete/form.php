...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Linkeds\Delete\form.php]
...
$r= $model->get_Linkeds($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Linkeds.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Linkeds.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Linkeds.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
