...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Series\Delete\form.php]
...
$r= $model->get_Series($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Series.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Series.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Series.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
